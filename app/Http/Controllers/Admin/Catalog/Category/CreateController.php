...
class CreateController extends Controller
{
  ...
  public function store(Request $request)
  {
    ...
    if (array_key_exists("banner",$inputs) && $request->hasFile('banner')) {
      $catalogCategory->banner = $this->featuredImageRetrivePath . $this->_imageUploadController->upload($request->file('banner'), $this->featuredImageStorePath);
    }else {
      $catalogCategory->banner = 'noImage.png';
    }
    ...
    if (array_key_exists("meta_image",$inputs) && $request->hasFile('meta_image')) {
      $catalogCategory->meta_image = $this->metaImageRetrivePath . $this->_imageUploadController->upload($request->file('meta_image'), $this->metaImageStorePath);
    }else {
      $catalogCategory->meta_image = 'noImage.png';
    }
